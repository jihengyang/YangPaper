...
% weight=weight.*repmat([kron(ones(N-1,1),[0;1]) kron(ones(N-1,1),[1;0])],1,N-1); % turn on for between ag and non-ag，等于表格里的overall
% weight=weight.*(1-kron(eye(N-1,N-1),ones(2,2))); % turn on for just between-province migration，等于省际移民成本变化-整体
